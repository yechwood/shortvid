...
public class MainActivity extends Activity {
    ...
    class FolderAdapter extends RecyclerView.Adapter<FolderAdapter.VH>{
        ...
        public void onBindViewHolder(VH h,int p){String f=list.get(p);int n=0;for(MediaItemData x:media)if(x.folder.equals(f))n++;h.title.setText(f);h.count.setText(n+" item"+(n==1?"":"s"));h.itemView.setOnClickListener(v->{folderMode=false;currentFolder=f;showHome();});}
